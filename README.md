...
| Pull/Fetch 摘要 | **Pull 合并完成**后弹窗展示**纯净变更摘要**（排除合并等操作提交；fetch 静默更新分支 ↓n 徽标与提交图，不弹摘要）：**作者 → 目录 → 文件** 三层分组；文件行带工作区大小与修改时间、行尾按钮一键打开或定位；不在工作区的文件（已被后续提交删除）按钮自动禁用并提示；同作者同文件多提交合并 ×N；重命名显示 `旧名 → 新名`；中文路径无八进制转义；`gitboard.pullFetchSummary` 开关，默认开启 |
...
```bash
code --install-extension gitboard-0.24.0.vsix
```
...
扩展面板（Ctrl+Shift+X）→ 右上角 `···` → **“从 VSIX 安装…”** → 选择 `gitboard-0.24.0.vsix` → 重新加载窗口。
...
**Pull 合并完成**后弹窗展示**纯净变更摘要**（排除 Branch/Merge 等操作提交）——回答"哪些人有哪些提交改了哪些文件"：

- **作者 → 目录 → 文件** 三层分组：作者头（最新提交在前）汇总其提交数/文件数；目录头显示相对路径一次（根目录显示仓库绝对路径）；组内只列文件名（完整不截断，过长换行）；
- 每行带工作区**大小与修改时间**，行尾两个按钮：**打开文件**、**在资源管理器中定位**（已被同批后续提交删除的文件按钮自动禁用并提示）；
- 同作者同文件多提交合并为一行（×N，悬停列出全部提交与说明）；重命名显示 `旧名 → 新名`；不在工作区的文件显示 `—`；
- 顶部汇总“N 个提交 · M 位作者 · K 个文件”；`gitboard.pullFetchSummary` 设置开关，默认开启；
- **Fetch 不弹摘要**：静默更新分支 ↓n 徽标与提交图（后台自动获取同此语义）；**网络操作互斥**——fetch/pull/push/标签推送任一进行中时，其余网络按钮禁用（未完成合并时 Pull/Push 亦预禁用）。
...
- **v0.22.0**（2026-09-10）：操作报错 AI 诊断与一键修复（[#8](https://github.com/Emon9426/VSCodeGitGUI/issues/8)，人审一键、机器管边界）——①**失败上下文透传**：操作失败携带 git 命令与退出码直达前端（此前在 runner 层被抹掉，诊断价值最高的字段缺失）；②**错误通知「AI 分析」入口**：操作失败弹常驻错误通知（重试+折叠 git 输出），Copilot 可用且非取消/停滞时附「AI 分析」按钮，一步直达诊断模态；③**AI 诊断模态**：三段式提示词（原因/解决步骤含可执行 git 命令/预防）、流式渲染、URL 凭证自动脱敏（`://user:token@` 掩码后才发送）；④**一键修复（机器管边界）**：诊断附 gitboard-fix JSON 修复方案，本地白名单独立分级 run/confirm/copy——不信任模型自报等级，`--force`/`clean`/`config`/未知旗标永远只给复制，confirm 级弹危险确认，执行步骤只传索引由宿主重校验防篡改（push 白名单仅扩 `--force-with-lease`，零新执行面）；⑤本版同时合入 v0.20.0 UI 反馈体系重构与 v0.21.0 多分支体验（#18/#24/#23）。
- **v0.21.0**（2026-09-10）：多分支仓库体验（[#24](https://github.com/Emon9426/VSCodeGitGUI/issues/24)，含 #23）——①**图形范围过滤**：工具栏「全部 / 本地 / 当前」一键切换提交图范围；默认「本地」= 本地分支及其上游的提交（远端领先状态照常入图），不再被仅存在于远端的分支与标签 tips 淹没（`gitboard.graphBranchScope` 可改默认）；②**分支可见性强化**：本地 `⑂` / 远程 `⇅` 徽标前缀（非色觉依赖）、HEAD 徽标描边醒目、存在同名本地分支时远程徽标降淡、当前分支主干走线加粗、侧栏「未推送 / 已有本地」小标；③**侧栏前缀分组**（[#23](https://github.com/Emon9426/VSCodeGitGUI/issues/23)）：本地与远程分支按 `/` 前缀恒分组，组头可折叠 + 计数，折叠状态跨会话保持（`gitboard.branchGroupByPrefix` 可关）；④**检出模糊搜索**：新检出选择器（工具栏「检出」按钮 / 命令面板 `GitBoard: 检出分支…`）支持分支名子序列模糊匹配与命中高亮，远程分支内联输入本地名一步检出；工具栏分支筛选下拉同步升级为该选择器（含范围项）。
- **v0.20.0**（2026-09-05）：Webview UI 反馈体系重构与交互阻塞设计（[#18](https://github.com/Emon9426/VSCodeGitGUI/issues/18)）——①**提交时间列修复**：窄窗口下时间不再被截断——列宽按当前字体实测保底（Canvas 测量，杜绝字体/DPI 差异），空间紧张自动降级为 `MM-DD HH:mm` 紧凑格式（完整时间悬停可见），表头「时间」与数据统一右对齐；②**通知（Notification）**：右下通知替代旧 toast——图标+标题+正文三档层级、分级停留（info 4s / 成功 5s / 警告 8s / **错误常驻**）、堆叠上限 4 条；**操作失败不再弹阻塞式确认框**，改为常驻错误通知（含「重试」按钮与可折叠的 git 输出）；成功类庆祝 toast 取消（进度行绿闪+按钮闪绿足够）；③**交互阻塞矩阵**：有依赖的操作互相阻塞并说明原因——冲突未解决时提交/AI 入口预禁用（悬停显示原因）、未完成合并时推送禁用、工具栏在途按钮**不再静默吞点击**（点击=入队去重，悬停显示「排队中·第 N 位」）、确认对话框首点即禁用防双击、Amend 期间 HEAD 前进自动退出修订模式；④**横幅（Banner）统一**：冲突/待完成合并/移动检测/推送询问/amend 提示/比对条七处样式归一为同一组件五个语义变体，推送询问条升级为带 sha+提交主题摘要；⑤**全站图标 SVG 化**：视图切换、工具栏、进度行、文件页等处的 emoji 字符全部替换为线性 SVG 图标（跨平台渲染一致）；⑥**对齐审计**：字号收敛为 5 档、面板内边距 12px 基准、图标命中区统一 20px、数字列等宽右对齐；设计令牌（间距/字号/语义色）落地为 CSS 变量。

**v0.19.3**（2026-09-05）：修复「冲突解决功能无效」——操作队列双道拆分与全链路可感知反馈（[#7](https://github.com/Emon9426/VSCodeGitGUI/issues/7)）——①**本地/网络双队列**：冲突解决、暂存、丢弃等秒级本地操作不再排在 Fetch/Pull/Push 等网络操作后面（此前网络停滞时队列被堵死，冲突按钮点击后 op 永久排队、表现为"点了没反应"）；双道并行产生的 index 锁瞬时冲突自动退避重试；②**入队即反馈**：本地操作从点击那刻起在进度行可见（排队中显示"第 N 位"，轮到执行切换为执行态），不再有零反馈窗口；③**autoFetch 后台让路**：网络道忙时后台自动获取整轮跳过，不再与用户操作争队列（顺带修复其绕过去重登记的缺陷 [#16](https://github.com/Emon9426/VSCodeGitGUI/issues/16)）；④**冲突行乐观态**：点「我的/对方的」行内立即转 ⏳ 并禁点按钮，完成/失败即解除，失败可重试；「完成合并」确认后按钮进入执行态防连点；⑤**统一推送收尾**：「拉取并推送」遇冲突时，全部解决并完成合并后弹确认条一次性推送全部新提交（推送永远显式确认，对齐 IDEA/SourceTree），中止合并则自动作废。冲突文件分类（文本/图片/Excel 等 NUL 二进制/万行超限）抽取为纯函数并有专项测试覆盖（含真实仓库三类文件全链路冒烟）。
...
| Pull/Fetch summary | After a **Pull merges** new commits, a popup lists the **pure changes** (merge ops excluded; fetch silently updates the ↓n badges and the commit graph — no popup): grouped **author → directory → file**: filename rows (never truncated) with working-tree size & mtime, inline open/reveal buttons; rows detected as absent from the working tree (deleted by later commits) get their buttons disabled with a tooltip; same-file commits merged into ×N rows, renames as `old → new`; toggle with `gitboard.pullFetchSummary`, on by default |
...
After a pull merges new commits, a **pure-change summary** (branch/merge ops excluded) pops up answering "who landed which commits touching which files": grouped **author → directory → file**; filename-only rows (never truncated) with working-tree size & mtime and inline **open / reveal** buttons (auto-disabled with a tooltip when the file was deleted by a later commit in the same batch); the same file touched several times merges into one ×N row (hover lists every commit); renames show `old → new`. Toggle with `gitboard.pullFetchSummary` (on by default). Fetch never pops a summary — it silently updates the behind badges and the commit graph. Network operations (fetch/pull/push/tag push) are mutually exclusive: while one is running, the other network buttons are disabled.
...
- **v0.22.0** (2026-09-10): AI error diagnosis & one-click fix ([#8](https://github.com/Emon9426/VSCodeGitGUI/issues/8), human-reviewed, machine-guarded) — ① **failure context passthrough**: failed ops now carry the git command and exit code to the UI (previously dropped at the runner layer — the most diagnostic fields were missing); ② **"AI Analyze" entry on error notifications**: failures raise a persistent error notification (retry + collapsible git output); when Copilot is available and the failure isn't a cancel/stall, an AI Analyze button opens the diagnosis modal in one step; ③ **diagnosis modal**: three-part prompt (cause / fix steps with executable git commands / prevention), streamed rendering, and URL credentials masked (`://user:token@` → `://***@`) before anything is sent; ④ **one-click fix with machine-enforced boundaries**: diagnoses may attach a gitboard-fix JSON plan executed against a local whitelist with independent tiers run/confirm/copy — the model's self-reported tier is never trusted, `--force`/`clean`/`config`/unknown flags always degrade to copy-only, confirm-tier steps raise the dangerous-action dialog, and steps are executed by index with host-side re-validation against tampering (the push whitelist only gains `--force-with-lease`; zero new execution surface); ⑤ this release also folds in the v0.20.0 feedback-system refactor and the v0.21.0 multi-branch experience (#18/#24/#23).
- **v0.21.0** (2026-09-10): multi-branch repo experience ([#24](https://github.com/Emon9426/VSCodeGitGUI/issues/24), includes #23) — ① **graph scope filter**: an All / Local / Current segmented control switches what the commit graph draws; the new default "Local" shows local branches plus their upstreams (remote-ahead commits included), instead of drowning in remote-only branch and tag tips (`gitboard.graphBranchScope` changes the default); ② **branch visibility**: local `⑂` / remote `⇅` chip prefixes (color-blind friendly), a highlighted HEAD chip, dimmed remote chips when a same-named local branch exists, a bolded current-branch trunk line, and "unpushed / local copy" markers in the sidebar; ③ **sidebar prefix grouping** ([#23](https://github.com/Emon9426/VSCodeGitGUI/issues/23)): local and remote branches group by the `/` prefix with collapsible, counted headers remembered across sessions (`gitboard.branchGroupByPrefix` to disable); ④ **fuzzy check-out picker**: a new branch picker (toolbar "Check out" button or command palette `GitBoard: Check Out Branch…`) with subsequence fuzzy matching and hit highlighting — local branches check out directly, remote branches ask for a local name inline; the toolbar branch filter dropdown was upgraded to the same picker (scope items included).
- **v0.20.0** (2026-09-05): webview feedback-system refactor + interaction blocking design ([#18](https://github.com/Emon9426/VSCodeGitGUI/issues/18)) — ① **commit time column fix**: no more truncation at narrow widths — the column's guaranteed width is measured at runtime against the actual font (Canvas measureText, immune to font/DPI differences), degrades to a compact `MM-DD HH:mm` format when space is tight (full value on hover), and the "Time" header is now right-aligned like the data; ② **Notifications**: bottom-right notifications replace the old toasts — icon + title + body hierarchy, level-based dwell (info 4s / success 5s / warn 8s / **errors persist**), stack limit of 4; **operation failures no longer open a blocking dialog** — they raise a persistent error notification with a Retry button and collapsible git output; success-celebration toasts are gone (progress-row flash + button flash suffice); ③ **interaction blocking matrix**: dependent actions block each other and explain why — commit/AI entries pre-disabled with a reason tooltip while conflicts are unresolved, push disabled while a merge is pending, in-flight toolbar buttons **no longer swallow clicks** (click = enqueue with dedup, hover shows "Queued · #n"), confirm dialogs disable on first click, and amend mode auto-exits when HEAD moves underneath it; ④ **unified banners**: conflict / pending-merge / move-detect / push-prompt / amend / compare bars all collapse into one component with five semantic variants; the push prompt now carries the sha + commit subject; ⑤ **all-emoji icons replaced with linear SVG** across view switcher, toolbar, progress row and file pages (consistent cross-platform rendering); ⑥ **alignment audit**: font sizes converge to 5 steps, panel padding on a 12px baseline, icon hit areas unified at 20px, numeric columns tabular-aligned; design tokens (spacing/type/semantic colors) land as CSS variables.
- **v0.19.3** (2026-09-05): fixed "conflict resolution unresponsive" — dual op lanes and end-to-end visible feedback ([#7](https://github.com/Emon9426/VSCodeGitGUI/issues/7)) — ① **local/net dual queues**: second-scale local ops (conflict resolution, staging, discarding) no longer queue behind Fetch/Pull/Push — previously a stalled network op wedged the single serial queue forever, so conflict buttons silently queued and never ran ("clicked, nothing happened"); transient `index.lock` races between lanes now retry with backoff; ② **feedback on enqueue**: local ops are visible in the progress row from the moment you click (queued shows "Queued · #n", switching to running when its turn comes) — no more zero-feedback windows; ③ **autoFetch yields**: the background auto-fetch skips its round entirely while the net lane is busy instead of competing with user actions (also fixes its dedup-registration bypass, [#16](https://github.com/Emon9426/VSCodeGitGUI/issues/16)); ④ **optimistic conflict rows**: clicking Mine/Theirs immediately switches the row to ⏳ with buttons disabled until the op settles (failures re-enable for retry), and Finish-merge enters a busy state to prevent double submits; ⑤ **unified push finish**: when "Pull & push" meets conflicts, completing the merge offers a confirmation bar that pushes all new commits in one go (push is always explicit, matching IDEA/SourceTree; aborting the merge cancels it). Conflict-file classification (text / NUL binaries like images & Excel / 10k-line over-limit) is now a pure function with dedicated tests, including a real-repo smoke across all three file kinds.
