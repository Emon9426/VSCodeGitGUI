# GitGraph — Git 可视化提交图 / Git Commit Graph for VS Code

**简体中文** | [English](#english-documentation)

GitGraph 是一个 VS Code 桌面版插件，以 SourceTree 风格的图形化提交历史为核心，浏览与操作 Git 仓库：彩色拓扑提交图、提交详情与差异对比、以及 Fetch / Pull / Push / 重置 / 切换分支等日常 GUI 操作。针对大仓库做了分页加载、虚拟滚动与分层渲染优化。
...
```bash
code --install-extension gitgraph-0.6.0.vsix
```
...
扩展面板（Ctrl+Shift+X）→ 右上角 `···` → **“从 VSIX 安装…”** → 选择 `gitgraph-0.6.0.vsix` → 重新加载窗口。
...
npm install
npm run package     # 产出 gitgraph-x.y.z.vsix
```

### 快速上手

1. 打开任意包含 Git 仓库的文件夹/工作区；
2. 点击**左侧活动栏的 GitGraph 图标**（或 `Ctrl+Alt+G`，或命令面板执行 `GitGraph: 打开提交图`）——主界面直接打开；
...
### 常用设置（`Ctrl+,` 搜索 "gitgraph"）

| 设置 | 默认 | 说明 |
| --- | --- | --- |
| `gitgraph.gitPath` | 自动检测 | git 可执行文件路径 |
| `gitgraph.commitPageSize` | 500 | 每页加载提交数（100–5000） |
| `gitgraph.maxAutoLoad` | 20000 | 自动加载上限 |
| `gitgraph.defaultPullStrategy` | merge | pull 策略：merge / rebase / ff-only |
| `gitgraph.dateFormat` | datetime | 时间格式：YYYY-MM-DD HH:mm:ss / 相对时间 / ISO |
| `gitgraph.rowHeight` | default | 行高：紧凑 20 / 标准 24 / 舒适 28 |
| `gitgraph.graphStyle` | curved | 走线风格：短半径圆角 / 直角折线 |
| `gitgraph.detailPanelPosition` | bottom | 详情面板位置：底部 / 右侧 |
| `gitgraph.language` | auto | 界面语言 |
| `gitgraph.fetchOnOpen` | true | 打开视图时自动 fetch |

### 从源码开发

```bash
npm install
npm run watch       # F5 调试（“运行 GitGraph 扩展”）
...
2. Click the **GitGraph icon in the Activity Bar** (or press `Ctrl+Alt+G`, or run `GitGraph: Open Commit Graph`);
...
- Search “gitgraph” in Settings for page size, auto-load limit, pull strategy, date format, row height, graph style, detail panel position, language, auto-fetch on open, and more.
...
```bash
npm install
npm run watch        # then press F5 (“运行 GitGraph 扩展”)
