...
import { DiffContentProvider, GITMAP_SCHEME, EMPTY_REF, gitmapUri } from './diffProvider';
...
export class GraphPanel {
  ...
  private readonly channel = vscode.window.createOutputChannel('GitMap');
  ...
  private constructor(private readonly context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
      ...
      vscode.workspace.onDidChangeConfiguration(e => {
        if (!e.affectsConfiguration('gitmap')) return;
...
<script nonce="${nonce}">${js}</script>
  ...
  private async route(cmd: string, args: any): Promise<any> {
    switch (cmd) {
      ...
      case 'ui:openFileAt': {
        const u = gitmapUri(this.currentRepoId!, path.basename(String(args.path)), String(args.sha), String(args.path));
      ...
      case 'ui:saveColWidths':
        await this.context.globalState.update('gitmap.colWidths', this.sanitizeColWidths(args.widths));
        return null;
      case 'ui:openSettings':
        await vscode.commands.executeCommand('workbench.action.openSettings', 'gitmap');
        return null;
      default:
  ...
  private async handleBootstrap(): Promise<void> {
    if (!this.executor) {
      const configured = vscode.workspace.getConfiguration('gitmap').get<string>('gitPath', '') || '';
      try {
  ...
  private async openDiffEditor(args: any): Promise<null> {
    ...
    const left = gitmapUri(this.currentRepoId!, fileName, leftRef, filePath);
    const right = worktree
      ? vscode.Uri.file(this.safeJoin(this.currentRoot(), filePath))
      : gitmapUri(this.currentRepoId!, fileName, sha, filePath);
  ...
  private readColWidths(): ColWidths | undefined {
    const w = this.context.globalState.get<Partial<ColWidths>>('gitmap.colWidths');
  ...
  private updateStatusBar(): void {
    const enabled = vscode.workspace.getConfiguration('gitmap').get<boolean>('showStatusBarItem', true);
    if (!this.statusBarItem) {
      this.statusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 100);
      this.statusBarItem.name = 'GitMap';
      this.statusBarItem.command = 'gitmap.open';
    }
    const st = this.lastState;
